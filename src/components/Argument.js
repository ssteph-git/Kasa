...
const Argument = function (props) {
    ...
    {
        //Description
        mesArguments.push(<p className={props.typeClass} key={Math.random()}>{props.description} </p>);
    }else{
        //Listes
        for (let i = 0; i < props.equipments.length; i++) {
            mesArguments.push(<p className="Liste" key={Math.random()}>{props.equipments[i]}</p>);
        }        
    }
    titreArgument = props.titre;//Titre de la catégorie duu composant
