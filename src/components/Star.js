...
const Star = function (props) {
    ...
    for (let i = 0; i < nbStarPlein; i++) {
        star.push(<i className="fa-solid fa-star starPlein" key={props.host+i+"starPlein"}></i>);
    }        
    for (let i = 0; i < nbStarVide; i++) {
        star.push(<i className="fa-solid fa-star starVide" key={props.host+i+"starVide"}></i>);
    }
